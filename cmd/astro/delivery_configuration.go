...
func newDeliveryInventoryCmd() *cobra.Command {
	...
	estate := &cobra.Command{
		Use:   "estate",
		Short: "Show estate-wide delivery health and compatibility",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runAPICommand(cmd, http.MethodGet, "/api/v1/delivery/estate/", nil, "")
		},
	}
	cmd.AddCommand(cluster, estate)
	return cmd
}
