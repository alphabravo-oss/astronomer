...
func registerProtectedRoutes(r chi.Router, cfg *config.Config, deps RouterDependencies, rateLimit func(appmiddleware.APIRateLimitClass) func(http.Handler) http.Handler) {
	...
	if deps.Resources != nil {
		...
		// 2FA admin override (migration 043). Superuser-only — used
		// when a user can't satisfy the password+code Disable gate
		// (lost authenticator). The handler reads the superuser flag
		// from the DB so a stale JWT can't bypass it.
		if deps.TOTP != nil {
			r.With(requireAuth(deps.JWT, deps.AuthQueries)).Post("/admin/users/{id}/disable-totp/", deps.TOTP.AdminForceDisable)
		}
	}
