...
func NewApp(ctx context.Context, cfg *config.Config, logger *slog.Logger) (*Server, error) {
	...
	deps := RouterDependencies{
		...
		// Sprint 075: read-only slug-coverage endpoint for the platform-
		// baseline cluster_template. Backed by the same queries surface;
		// the handler uses sqlc.ResolveChartByName (hand-written shim in
		// internal/db/sqlc/catalog_coverage_ext.sql.go).
