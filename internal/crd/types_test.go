...
func TestSchemeContainsOnlyRetainedManagementKinds(t *testing.T) {
	...
	for _, removed := range []string{"ClusterBaseline", "ComponentBundle", "GitOpsTarget"} {
		if _, err := scheme.New(GroupVersion.WithKind(removed)); err == nil {
			t.Fatalf("removed kind %s is still registered", removed)
		}
	}
}
...
func TestClusterDeepCopyDoesNotAliasCollections(t *testing.T) {
	...
	if original.Spec.Labels["a"] != "b" || original.Spec.ProjectRefs[0] != "p" || original.Spec.AdoptionPolicy.AllowedManagementModes[0] != "flux" {
		t.Fatal("deep copy aliases source")
	}
}
