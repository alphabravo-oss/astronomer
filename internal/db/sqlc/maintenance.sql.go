...
const getDeferredOperationForUpdate = `-- name: GetDeferredOperationForUpdate :one
SELECT id, window_id, operation_type, operation_spec, target_cluster_id,
       target_project_id, status, deferred_until, expires_at, requested_by,
       last_error, dispatched_at, created_at, updated_at
FROM deferred_operations
...
func (q *Queries) GetDeferredOperationForUpdate(ctx context.Context, id uuid.UUID) (DeferredOperation, error) {
	row := q.db.QueryRow(ctx, getDeferredOperationForUpdate, id)
	var i DeferredOperation
	err := row.Scan(
		&i.ID,
		&i.WindowID,
...
const getMaintenanceWindowForUpdate = `-- name: GetMaintenanceWindowForUpdate :one
SELECT id, name, description, mode, cron_open, duration_minutes, timezone,
       cluster_selector, operation_types, on_block, enabled,
       created_by, created_at, updated_at
FROM maintenance_windows
...
func (q *Queries) GetMaintenanceWindowForUpdate(ctx context.Context, id uuid.UUID) (MaintenanceWindow, error) {
	row := q.db.QueryRow(ctx, getMaintenanceWindowForUpdate, id)
	var i MaintenanceWindow
	err := row.Scan(
		&i.ID,
		&i.Name,
...
func (q *Queries) ListPendingDeferredOperations(ctx context.Context, arg ListPendingDeferredOperationsParams) ([]DeferredOperation, error) {
	rows, err := q.db.Query(ctx, listPendingDeferredOperations, arg.Now, arg.BatchLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
