...
type ConfigurationTemplateMutationTx interface {
	audit.OutboxQuerier
	CreateDeliveryConfigurationTemplate(context.Context, sqlc.CreateDeliveryConfigurationTemplateParams) (sqlc.DeliveryConfigurationTemplate, error)
	UpdateDeliveryConfigurationTemplate(context.Context, sqlc.UpdateDeliveryConfigurationTemplateParams) (sqlc.DeliveryConfigurationTemplate, error)
	DeleteDeliveryConfigurationTemplate(context.Context, sqlc.DeleteDeliveryConfigurationTemplateParams) (uuid.UUID, error)
...
type configurationTemplateRunTxFunc func(context.Context, func(ConfigurationTemplateMutationTx) error) error

type ConfigurationTemplateHandler struct {
	queries ConfigurationTemplateQueries
	runTx   configurationTemplateRunTxFunc
}
...
func (h *ConfigurationTemplateHandler) SetRunTx(runTx configurationTemplateRunTxFunc) {
	if h != nil {
		h.runTx = runTx
	}
}

func (h *ConfigurationTemplateHandler) TransactionalAuditWired() bool {
...
func (h *ConfigurationTemplateHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	paging.Write(w, items, paging.Exact(count, int(limit), int(offset), len(items)))
}
...
func (h *ConfigurationTemplateHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	params := sqlc.CreateDeliveryConfigurationTemplateParams{
		...
		CreatedBy: reqctx.UserUUID(r.Context()),
	}
	row, err := executeMutation(r, h.runTx,
		func(q ConfigurationTemplateMutationTx) (sqlc.DeliveryConfigurationTemplate, error) {
			return q.CreateDeliveryConfigurationTemplate(r.Context(), params)
		},
		func(row sqlc.DeliveryConfigurationTemplate) deliveryAuditEvent {
			return deliveryAuditEvent{action: "delivery.configuration_template.created", resourceType: "delivery_configuration_template", resourceID: row.ID.String(), resourceName: row.Name, status: http.StatusCreated, detail: map[string]any{"renderer": row.Renderer}}
		})
	if err != nil {
		respondDatabaseError(w, err)
		return
...
func (h *ConfigurationTemplateHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	params := sqlc.UpdateDeliveryConfigurationTemplateParams{
		ProjectID: projectID, ID: id, Name: input.Name, Description: input.Description, Renderer: input.Renderer,
		ValuesDocument: input.Values, Patches: input.Patches, SecretRefs: input.SecretRefs,
		UpdatedBy: reqctx.UserUUID(r.Context()), Generation: generation,
	}
	row, err := executeMutation(r, h.runTx,
		func(q ConfigurationTemplateMutationTx) (sqlc.DeliveryConfigurationTemplate, error) {
			return q.UpdateDeliveryConfigurationTemplate(r.Context(), params)
		},
		func(row sqlc.DeliveryConfigurationTemplate) deliveryAuditEvent {
			return deliveryAuditEvent{action: "delivery.configuration_template.updated", resourceType: "delivery_configuration_template", resourceID: row.ID.String(), resourceName: row.Name, status: http.StatusOK, detail: map[string]any{"generation": row.Generation}}
		})
...
func (h *ConfigurationTemplateHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	params := sqlc.DeleteDeliveryConfigurationTemplateParams{ProjectID: projectID, ID: id, Generation: generation}
	_, err = executeMutation(r, h.runTx,
		func(q ConfigurationTemplateMutationTx) (uuid.UUID, error) {
			return q.DeleteDeliveryConfigurationTemplate(r.Context(), params)
		},
		func(deletedID uuid.UUID) deliveryAuditEvent {
			return deliveryAuditEvent{action: "delivery.configuration_template.deleted", resourceType: "delivery_configuration_template", resourceID: deletedID.String(), status: http.StatusNoContent}
		})
	if errors.Is(err, pgx.ErrNoRows) {
