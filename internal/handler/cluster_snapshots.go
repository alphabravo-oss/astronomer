...
func parseClusterAndSnapshotIDs(w http.ResponseWriter, r *http.Request) (uuid.UUID, uuid.UUID, bool) {
	...
	if err != nil {
		RespondError(w, http.StatusBadRequest, "invalid_id", "Invalid snapshot ID")
		return uuid.Nil, uuid.Nil, false
	}
...
func (h *ClusterSnapshotsHandler) ListSnapshots(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		RespondError(w, http.StatusInternalServerError, "list_error", "Failed to list snapshots")
		return
	}
...
func (h *ClusterSnapshotsHandler) GetSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || row.ClusterID != clusterID {
		RespondError(w, http.StatusNotFound, "not_found", "Snapshot not found")
		return
	}
...
func (h *ClusterSnapshotsHandler) CreateSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	if !validVeleroResourceName(veleroName) {
		RespondError(w, http.StatusBadRequest, "invalid_body", "velero_name must be a valid RFC 1123 subdomain (1-253 chars)")
		return
	}
	...
	if err != nil {
		RespondError(w, http.StatusInternalServerError, "create_error", "Failed to create snapshot row")
		return
	}
...
func (h *ClusterSnapshotsHandler) DeleteSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.queries.DeleteClusterSnapshot(r.Context(), snapshotID); err != nil {
		RespondError(w, http.StatusInternalServerError, "delete_error", "Failed to delete snapshot row")
		return
	}
...
func (h *ClusterSnapshotsHandler) CreateRestore(w http.ResponseWriter, r *http.Request) {
	...
	if snapshot.Phase != "Completed" && snapshot.Phase != "PartiallyFailed" {
		RespondError(w, http.StatusConflict, "snapshot_not_ready", "Snapshot is not yet Completed; cannot restore")
		return
	}
	...
	if strings.TrimSpace(req.TargetClusterID) != "" {
		...
		if err != nil {
			RespondError(w, http.StatusBadRequest, "invalid_body", "Invalid target_cluster_id")
			return
		}
		targetID = parsed
	}
	...
	if err != nil {
		RespondError(w, http.StatusNotFound, "not_found", "Target cluster not found")
		return
	}
	...
	if targetID != clusterID {
		...
		if vErr != nil {
			RespondError(w, http.StatusBadGateway, "velero_unreachable", fmt.Sprintf("could not check Velero on target cluster: %v", vErr))
			return
		}
		if len(bsls) == 0 {
			RespondError(w, http.StatusConflict, "velero_missing_on_target", "Target cluster has no Velero BackupStorageLocation; install Velero before cross-cluster restore")
			return
		}
	}
	...
	if err != nil {
		RespondError(w, http.StatusInternalServerError, "create_error", "Failed to create restore row")
		return
	}
...
func (h *ClusterSnapshotsHandler) CreateSchedule(w http.ResponseWriter, r *http.Request) {
	...
	if !validVeleroResourceName(strings.ToLower(req.Name)) {
		RespondError(w, http.StatusBadRequest, "invalid_body", "name must be a valid DNS subdomain")
		return
	}
	if _, err := parseCronExpression(req.CronSchedule); err != nil {
		RespondError(w, http.StatusBadRequest, "invalid_body", fmt.Sprintf("invalid cron_schedule: %v", err))
		return
	}
	enabled := true
	if req.Enabled != nil {
	...
	if err != nil {
		RespondError(w, http.StatusInternalServerError, "create_error", "Failed to create schedule (name conflict?)")
		return
	}
...
func (h *ClusterSnapshotsHandler) VeleroStatus(w http.ResponseWriter, r *http.Request) {
	...
	if h.requester == nil {
		RespondError(w, http.StatusServiceUnavailable, "tunnel_unwired", "Tunnel requester not configured")
		return
	}
