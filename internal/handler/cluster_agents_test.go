...
type fakeClusterAgentLiveRequester struct {
	responses map[string]*protocol.K8sResponsePayload
	calls     []string
}

func (f *fakeClusterAgentLiveRequester) Do(_ context.Context, _ string, method, path string, _ []byte, _ map[string]string) (*protocol.K8sResponsePayload, error) {
...
func (f *fakeClusterAgentQuerier) CountClusters(context.Context) (int64, error) {
	return int64(len(f.clusters)), nil
}

func (f *fakeClusterAgentQuerier) GetClusterByID(_ context.Context, id uuid.UUID) (sqlc.Cluster, error) {
...
func (f *fakeClusterAgentQuerier) ListLatestConnectionsByClusters(_ context.Context, clusterIDs []uuid.UUID) ([]sqlc.AgentConnection, error) {
...
func (f *fakeClusterAgentQuerier) ListClusterConditions(_ context.Context, clusterID uuid.UUID) ([]sqlc.ClusterCondition, error) {
	return f.conditions[clusterID], nil
}

func (f *fakeClusterAgentQuerier) CreateAgentLifecycleOperation(_ context.Context, arg sqlc.CreateAgentLifecycleOperationParams) (sqlc.AgentLifecycleOperation, error) {
...
func TestClusterAgentListSummarizesConnectedDegradedDisconnectedAgents(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/cluster-agents/", nil)
	h.List(rr, req)
...
func TestClusterAgentListWithoutQuerierReturns503(t *testing.T) {
	h := NewClusterAgentHandler(nil)
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/api/v1/cluster-agents/", nil)
	h.List(rr, req)
...
func TestClusterAgentDiagnosticsReturnsRedactedTriagePayload(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/cluster-agents/"+clusterID.String()+"/diagnostics/", nil)
...
func TestClusterAgentDiagnosticsBundleDownloadsRedactedJSON(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("cluster_id", clusterID.String())
	req := httptest.NewRequest(http.MethodGet, "/api/v1/cluster-agents/"+clusterID.String()+"/diagnostics/bundle/", nil)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	rr := httptest.NewRecorder()
...
func TestClusterAgentDiagnosticsBundleStrictlyRedactsLiveSensitiveValues(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	h.SetK8sRequester(&fakeClusterAgentLiveRequester{responses: map[string]*protocol.K8sResponsePayload{
...
func TestClusterAgentDiagnosticsIncludesLiveAgentSnapshot(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	h.now = func() time.Time { return now }
	selector := url.QueryEscape("app.kubernetes.io/name=astronomer-agent,app.kubernetes.io/component=agent")
	requester := &fakeClusterAgentLiveRequester{responses: map[string]*protocol.K8sResponsePayload{
		"GET /apis/apps/v1/namespaces/astronomer-system/deployments/astronomer-agent": k8sJSONResponse(http.StatusOK, map[string]any{
...
func TestClusterAgentSelfTestPassesForHealthyConnectedAgent(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("cluster_id", clusterID.String())
	req := httptest.NewRequest(http.MethodPost, "/api/v1/cluster-agents/"+clusterID.String()+"/self-test/", nil)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	rr := httptest.NewRecorder()
	h.SelfTest(rr, req)
	if rr.Code != http.StatusOK {
	...
	for _, name := range []string{"agent_connection", "heartbeat_freshness", "ping_freshness", "privilege_profile", "compatibility", "live_diagnostics", "cluster_conditions"} {
...
func TestClusterAgentSelfTestFailsForDisconnectedBlockedAgent(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
	...
	statuses := selfTestCheckStatuses(envelope.Data.Checks)
	for _, name := range []string{"agent_connection", "heartbeat_freshness", "ping_freshness", "compatibility", "live_diagnostics", "cluster_conditions"} {
...
func TestClusterAgentUpgradePlanReadyForConnectedRemoteAgent(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
...
func TestClusterAgentUpgradePlanAcceptsRolloutControls(t *testing.T) {
	...
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("cluster_id", clusterID.String())
	req := httptest.NewRequest(http.MethodPost, "/api/v1/cluster-agents/"+clusterID.String()+"/upgrade-plan/", strings.NewReader(body))
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	rr := httptest.NewRecorder()
	h.UpgradePlan(rr, req)
	if rr.Code != http.StatusOK {
...
func TestClusterAgentUpgradePlanBlocksDisconnectedLocalAgent(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
...
func TestClusterAgentUpgradeQueuesLifecycleOperation(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	fake := &fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
			{
				ID:          clusterID,
				Name:        "prod",
	...
	if idem.Scope != "agent_lifecycle:user:anonymous:POST:/api/v1/cluster-agents/"+clusterID.String()+"/upgrade/" || idem.IdempotencyKey != "agent-upgrade-retry-1" {
...
func TestClusterAgentUpgradeDoesNotQueueBlockedPlan(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	fake := &fakeClusterAgentQuerier{
		clusters: []sqlc.Cluster{
...
func TestClusterAgentOperationsListsLifecycleHistory(t *testing.T) {
	now := time.Date(2026, 6, 13, 12, 0, 0, 0, time.UTC)
	clusterID := uuid.New()
	h := NewClusterAgentHandler(&fakeClusterAgentQuerier{
	...
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("cluster_id", clusterID.String())
	req := httptest.NewRequest(http.MethodGet, "/api/v1/cluster-agents/"+clusterID.String()+"/operations/", nil)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	rr := httptest.NewRecorder()
	h.Operations(rr, req)
	if rr.Code != http.StatusOK {
...
func TestAgentPrivilegeProfileSelfTestPasses(t *testing.T) {
	for _, profile := range []string{
		...
		c := agentPrivilegeProfileSelfTestCheck(clusterAgentItem{PrivilegeProfile: profile})
		if c.Status != "passed" {
	...
	custom := agentPrivilegeProfileSelfTestCheck(clusterAgentItem{PrivilegeProfile: agenttemplate.PrivilegeProfileCustom})
	if custom.Status != "warning" {
		t.Fatalf("custom profile should warn, got %q: %s", custom.Status, custom.Message)
	}

	implicit := agentPrivilegeProfileSelfTestCheck(clusterAgentItem{})
