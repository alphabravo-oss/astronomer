	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := executeMutation(r, h.runTx,
				...
				func(row sqlc.CloudCredential) mutationAuditEvent {
					return mutationAuditEvent{action: "cloud_credentials.created", resourceType: "cloud_credential", resourceID: row.ID.String(), status: http.StatusCreated}
				})
...
func TestEveryCloudCredentialMutationUsesTransactionalExecutor(t *testing.T) {
	paths, err := filepath.Glob("cloud_credentials*.go")
	if err != nil {
		t.Fatal(err)
	}
