...
var commandDescriptors = []CommandDescriptor{
	...
	{ID: "agents", Version: commandWorkflowVersion, Name: "agents", Label: "Cluster agents", Description: "Assess downstream agent connectivity using Astronomer-owned control-plane telemetry only.", Category: "Investigate", Execution: "agent", Effect: "read", RequiredMode: ModeReadOnly, Example: "/agents", Objective: "Assess Astronomer cluster agents using management-plane connection metadata, heartbeat history, versions, ingestion, tunnel ownership, and cross-cluster patterns only; do not query downstream Kubernetes.", Sections: []string{"Cluster-agent summary", "Agents needing attention", "Connection and tunnel evidence", "Recommended operator checks"}},
...
var commandToolWorkflows = map[string]commandToolWorkflow{
	...
	"agents":      {Maximum: 8, First: []string{"astronomer.cluster_agents.summary", "astronomer.tunnel.health"}},
