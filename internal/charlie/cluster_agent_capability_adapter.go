...
type clusterAgentCapabilityQueries interface {
...
// ClusterAgentCapabilityAdapter reads only Astronomer-owned PostgreSQL operational
// metadata. It has no tunnel, Kubernetes, bridge, or credential dependency.
type ClusterAgentCapabilityAdapter struct{ queries clusterAgentCapabilityQueries }

func NewClusterAgentCapabilityAdapter(queries clusterAgentCapabilityQueries) (*ClusterAgentCapabilityAdapter, error) {
	if queries == nil {
		return nil, fmt.Errorf("Charlie cluster-agent capability store is unavailable")
	}
	return &ClusterAgentCapabilityAdapter{queries: queries}, nil
}

func (a *ClusterAgentCapabilityAdapter) Execute(ctx context.Context, capability CapabilityDescriptor, arguments map[string]json.RawMessage) (json.RawMessage, error) {
	switch capability.Name {
	case "astronomer.cluster_agents.summary":
		stale := int64Argument(arguments, "stale_after_seconds", 300)
		row, err := a.queries.CharlieClusterAgentSummary(ctx, int32(stale))
		if err != nil {
			return nil, err
		}
		pattern := "healthy"
		...
		if row.DisconnectedClusters > 1 || row.StaleHeartbeats > 1 {
			pattern = "multi_cluster_event"
			clusterAgents, listErr := a.queries.CharlieClusterAgentList(ctx, sqlc.CharlieClusterAgentListParams{PageLimit: 100})
			if listErr == nil {
				environments, regions, affected := map[string]struct{}{}, map[string]struct{}{}, int64(0)
				for _, item := range clusterAgents {
	...
	case "astronomer.cluster_agents.list":
		...
		rows, err := a.queries.CharlieClusterAgentList(ctx, sqlc.CharlieClusterAgentListParams{
	...
	case "astronomer.cluster_agents.get":
		...
		row, err := a.queries.CharlieClusterAgentGet(ctx, clusterID)
		if err != nil {
			return nil, err
		}
	...
	default:
		return nil, fmt.Errorf("unsupported cluster-agent capability")
	}
}

func (a *ClusterAgentCapabilityAdapter) Verify(context.Context, CapabilityDescriptor, map[string]json.RawMessage, json.RawMessage) (bool, error) {
	return true, nil
}
