...
# The sqlc package may contain sqlc output, deterministically generated legacy
# extensions, and tests. Handwritten production Go in this directory is a
# source-ownership defect even when `sqlc generate` happens not to touch it.
manual_files="$(python3 - <<'PY'
...
for path in sorted(Path("internal/db/sqlc").glob("*.go")):
    if path.name.endswith("_test.go"):
        continue
    first = path.read_text(encoding="utf-8").splitlines()[:1]
    if not first or first[0] not in allowed:
        print(path)
PY
)"
if [[ -n "$manual_files" ]]; then
  printf 'handwritten production Go is forbidden in internal/db/sqlc:\n%s\n' "$manual_files" >&2
  exit 1
fi
