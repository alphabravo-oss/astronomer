...
const INTENTIONAL_NON_JSON_ADAPTERS = [
  ...
  [
    "frontend/src/components/clusters/cluster-shell.tsx",
    "WebSocket",
    "cluster shell",
  ],
  [
    "frontend/src/components/workloads/pod-terminal.tsx",
    "WebSocket",
    "pod terminal",
  ],
  [
    "frontend/src/lib/api/settings-compliance-export.ts",
    "fetch(downloadUrl)",
    "pre-signed compliance export download",
  ],
];
...
for (const [file, count] of actual) {
  const budget = INTENTIONAL_API_ADAPTER_BUDGET.get(file);
  if (budget === undefined)
    failures.push(`${file}: ${count} unclassified raw transport call(s)`);
  else if (count > budget)
    failures.push(`${file}: raw transport count grew ${budget} -> ${count}`);
}
for (const [file, budget] of INTENTIONAL_API_ADAPTER_BUDGET) {
  const count = actual.get(file) ?? 0;
  if (count < budget)
    failures.push(
      `${file}: raw transport count fell ${budget} -> ${count}; lower the ratchet baseline`,
    );
}
...
for (const [file, marker, reason] of INTENTIONAL_NON_JSON_ADAPTERS) {
  ...
  if (
    ...
    !fs.readFileSync(absolute, "utf8").includes(marker)
  ) {
    failures.push(
      `${file}: stale ${reason} exception (missing marker ${JSON.stringify(marker)})`,
    );
  }
}
...
console.log(
  `Frontend raw transport inventory: ${intentionalCount} intentional Kubernetes adapter call(s); ordinary JSON calls must use generated operations.`,
);
