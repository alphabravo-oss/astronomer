...
| Argo CD Kubernetes API proxy | `/api/v1/internal/argocd/clusters/{cluster_id}/k8s/*` | `internal/server/routes.go`, `internal/tunnel/proxy.go` | Adopted cluster API via agent | Cluster-scoped Argo proxy token only | Mutating requests emit `argocd.k8s_proxy.forwarded`; downstream actions also visible through Argo/Application history | Covered |
...
| Argo CD internal cluster proxy token scope and mutation audit | `TestArgoCDInternalK8sProxyRequiresClusterScopedToken`, `TestArgoCDInternalK8sProxyMutationsAreAudited` |
