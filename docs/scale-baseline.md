...
|---|---|---|---|---|
| 2026-09-17 | `0e0a321d` exact digest-pinned images | local shared 16-CPU/32-GiB host; k3s; one server, one worker, bundled PostgreSQL 16, Valkey 8 | Estate-100 completed 30 minutes with passing endpoint latency, bounded resources, 100/100 reconnect recovery, and conservation of all 8,964 accepted audit intents, but the DB-pool empty-acquire rate was 12.030/s (limit 0.100/s). The run also exposed a 473.55-RPS scheduler shortfall, one window-boundary audit rejection, and state events continuing into drain; all require an exact-candidate rerun after the recorded fixes. | [`scale-evidence/estate-100-2026-09-17-local-fail.md`](scale-evidence/estate-100-2026-09-17-local-fail.md) |
