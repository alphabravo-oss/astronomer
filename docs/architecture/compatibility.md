...
This table describes the `1.1.x` release line. Install mode is **fresh and upgrade**. Supported upgrades begin at schema 1 and migrate to schema 26.
...
| PostgreSQL | 16, 17; TLS required in production; schemas 1-26 supported for upgrade |
| Browsers | Chrome 130+, Edge 130+, Firefox 128+, Safari 18.0+; current and previous stable at release |

Unknown future agent majors are rejected before authentication and work dispatch. The release supports 2 adjacent release lines; deprecated agents remain connected only inside the declared compatibility window.

CONNECT admission also requires: `delivery.assignments.v2`, `delivery.status.v2`, `delivery.system.v2`.
