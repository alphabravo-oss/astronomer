...
<!-- api-contract-evidence: operations=831 mounted_routes=805/805 request_bindings=160 provisional_request_shapes=0 actionable_202=107 async_202_exceptions=5 -->
...
| Multi-cluster grouping and tenancy | Partial | Projects, cluster groups, namespace assignments, global/project/cluster/namespace RBAC, effective-permission preview, custom-role edit/duplicate/delete, curated project-role template application, audited directory search, and permission-aware navigation. See the [RBAC permission contract](rbac-permission-contract.md) and `internal/rbac`. | Pre-login principal discovery is available for LDAP connectors; other connector types report that discovery is unsupported instead of fabricating results. Namespace-scoped behavior also depends on an agent profile capable of enforcing the requested operation. |
| GitOps source and delivery lifecycle | Complete | Immutable Git/OCI/Helm source identities, bundle versions, bounded per-target values/patch overrides, frozen placement snapshots and override digests, approvals, maintenance windows, rollout strategies, assignment generations, normalized status, rollback, and local Flux reconciliation. See [delivery ADR](architecture/decisions/flux-native-delivery.md), [state contract](control-plane-state-contract.md), and `internal/handler/delivery_*`. | Branch tracking is intentionally excluded; there is no Rancher Fleet API or compatibility surface. |
...
| Workload and node operations | Partial | Scale, restart, pod deletion, bulk deletion from explorer tables with per-object authorization/audit and partial-failure results, logs, exec, cordon, uncordon, label/annotation/taint changes, and policy-aware drain. Requests are API-driven and audited. | Bulk scale/restart and node mutations remain single-object operations. Behavior is also limited by the adopted agent's declared capability and privilege profile. |
...
| Security posture and policy | Partial | Durable CIS scan lifecycle, image vulnerability history, Gatekeeper constraints, Pod Security, network policy, API-server allowlist, registry and service-mesh posture. | API-server allowlist monitoring/enforcement is unavailable for self-managed providers; the UI reports that limitation instead of presenting a synthetic baseline. Findings do not replace human risk acceptance. |
| Identity and lifecycle management | Partial | Local identity, OIDC/OAuth connectors, TOTP and recovery, password policy, API tokens, group mapping, SCIM Users/Groups, session revocation, lockout, quotas, and audited inactive-user retention. | SSO requires installing the Dex tool (`dex_bundled_enabled` defaults off), and connector-specific upstream availability remains external. |
...
| Backup, restore, and disaster recovery | Partial | Management-plane backup destinations, restore drills, Velero cluster workflows, key ownership guidance, PostgreSQL failover/restore runbooks, and operation history. | Management backups and control-plane snapshots default off. A release claim requires the operator's own restore rehearsal and storage durability evidence. |
| API automation | Complete | The quality-checked contract covers every mounted route, binds Go request shapes with zero drift and zero provisional/propertyless request shapes, and generates TypeScript and Go clients plus the embedded specification. The machine-checked evidence marker above records current operation, route, binding and response counts. Actionable `202` mutations require bounded idempotency and durable typed receipts/status resources; explicitly locked exceptions cover trusted ingest, privacy acknowledgement or interactive message acceptance. See the generated [OpenAPI contract](openapi.yaml) and `scripts/openapi-quality.mjs`. | The raw Kubernetes proxy remains an intentionally opaque adapter rather than a domain schema; its policy, authorization, and audit boundaries are public and tested. |
...
| Extension model | Partial | Signed/validated declarative extension manifests, governed mounts, capability checks, constrained proxy bridge, and uninstall/reinstall lifecycle. | The feature defaults off while the marketplace is incomplete. Arbitrary in-process backend plugins are not supported. |
