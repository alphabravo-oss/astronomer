...
| [020 — Cluster explorer nav: discovery + CRD groups](./020-cluster-explorer-nav-discovery-and-crd-groups.md) | P2 · L | **IN PROGRESS** on `advisor/020-explorer-nav`, based on integrated `642ab6c5`; discovery/subgroups/stars and migration 065 implemented; combined verification running | `ifHaveGroup` gating, Gateway API folded into Service Discovery, dynamic More Resources subgroups, starred types (adds `starred_types` pref). |
...
| [025 — Guided form depth and table parity](./025-guided-form-depth-and-table-parity.md) | P2 · L | TODO (after 024 step 6, 021) | `ArrayField` for containers/rules/volumes/env/ports/paths/tolerations/affinity, group-by-namespace, bulk restart/scale, generic related resources, `questions.yaml` ADR spike. |
...
These plans touch `frontend/` (plus two small `UserPreferences` schema additions) and do not overlap Plan 016's remaining scale/external-evidence gates. If a Plan 016 phase touches the same frontend files, 016 wins and the UI plan's drift check will flag it.
