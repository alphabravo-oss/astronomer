...
| 4 | `operator-navigation-contract.spec.ts` verifies all eight Delivery destinations retain project context and unique active links on cluster/global pages; restricted-role specifications distinguish list grants, read grants and inventory scope. The installed-tools Onboarding template state and exact Shared stacks active state have additional test-only coverage queued for the parent’s combined run (`500bd0b1`); neither is claimed as passed in these retained runs. |
...
Test-only checkpoint `5c31cf25` adds keyboard selection of both identically labelled Overview commands, multi-container/tail/log-filter URL refresh, invalid/Exec URL non-execution, and originating workload403/404 with browser Back recovery. These cases also await the parent’s combined run. Missing/denied alert-destination variants are assigned to the independent browser executor. These queued cases are not included in the passing counts above.

The parent’s first combined frontend enterprise run passed lint/typecheck and1788 unit tests, with one stale catalog search expectation failing. Checkpoint `5646350e` updates that expectation to require server-side search while retaining project isolation; its targeted file passed all five tests. Complete enterprise rerun remains parent-owned.
