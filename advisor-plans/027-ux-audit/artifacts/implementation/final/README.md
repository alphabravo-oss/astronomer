...
Raw verifier logs retain their exact bytes, including reporter whitespace, so their committed hashes remain valid. The artifact directory disables Git whitespace diagnostics only for those generated `.log` files; handwritten source and documentation checks remain unchanged.
