...
- Name: review.spec.ts >> reproduce namespace disagreement on custom resources
- Location: advisor-plans/027-ux-audit/review.spec.ts:95:1
...
Locator: getByRole('button', { name: 'Namespace scope: team-a', exact: true })
Expected: visible
Timeout: 30000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByRole('button', { name: 'Namespace scope: team-a', exact: true }) with timeout 30000ms
  - waiting for getByRole('button', { name: 'Namespace scope: team-a', exact: true })
...
- complementary:
  ...
  - navigation:
    ...
    - button "More Resources" [expanded]
    - paragraph: Built-in
    - link "Custom Resources":
      - /url: /dashboard/clusters/c-smoke-1/custom-resources
    - link "CRDs":
      - /url: /dashboard/clusters/c-smoke-1/crds
    - button "Star CRDs"
    - link "Endpoints":
      - /url: /dashboard/clusters/c-smoke-1/endpoints
    - button "Star Endpoints"
    - link "ReplicaSets":
      - /url: /dashboard/clusters/c-smoke-1/replicasets
    - button "Star ReplicaSets"
    - link "Mirrored Resources":
      - /url: /dashboard/clusters/c-smoke-1/resources
    - paragraph: Cert Manager
    - link "Certificate":
      - /url: /dashboard/clusters/c-smoke-1/custom-resources/cert-manager.io/v1/certificates
    - button "Star Certificate"
  - link "Documentation":
    - /url: /astronomer-docs/
...
- main:
  - link "Back":
    - /url: /dashboard/clusters/c-smoke-1/custom-resources
  - heading "certificates" [level=1]
  - paragraph: cert-manager.io/v1
  - textbox "Search this page of certificates..."
  - button "Columns"
  - grid:
    - row "Name Namespace Age Actions":
      - columnheader "Name"
      - columnheader "Namespace"
      - columnheader "Age"
      - columnheader "Actions"
    - row "cert-team-a team-a - Open actions menu":
      - gridcell "cert-team-a":
        - link "cert-team-a":
          - /url: /dashboard/clusters/c-smoke-1/custom-resources/cert-manager.io/v1/certificates/team-a/cert-team-a
      - gridcell "team-a"
      - gridcell "-"
      - gridcell "Open actions menu":
        - button "Open actions menu"
    - row "cert-team-b team-b - Open actions menu":
      - gridcell "cert-team-b":
        - link "cert-team-b":
          - /url: /dashboard/clusters/c-smoke-1/custom-resources/cert-manager.io/v1/certificates/team-b/cert-team-b
      - gridcell "team-b"
      - gridcell "-"
      - gridcell "Open actions menu":
        - button "Open actions menu"
  - text: Server page 1; up to 50 resources
...
```ts
  ...
  9   |   await installStubs(page);
  10  |   await seedAuth(context, page, adminStoreUser);
  11  |   await page.clock.setFixedTime(new Date('2026-09-24T12:00:00Z'));
  12  | });
  13  | 
  14  | test('measure current source shell at five widths', async ({ page }, info) => {
  15  |   const findings: unknown[] = [];
  16  |   for (const width of [390, 412, 768, 1024, 1280]) {
  17  |     await page.setViewportSize({ width, height: 900 });
  18  |     await page.goto(`/dashboard/clusters/${SMOKE_CLUSTER_ID}`);
  19  |     await expect(page.getByTestId('app-shell')).toBeVisible();
  20  |     await expect(page.getByRole('heading', { name: 'Smoke East', exact: true })).toBeVisible();
  21  |     const controls = await page.locator('header button').evaluateAll((buttons) => buttons.map((el) => {
  22  |       const r = el.getBoundingClientRect();
  23  |       return { label: el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent?.trim(),
  24  |         x: Math.round(r.x), right: Math.round(r.right), width: Math.round(r.width),
  25  |         visible: r.width > 0 && r.height > 0, outside: r.x < 0 || r.right > innerWidth };
  26  |     }));
  27  |     findings.push({ width, controls });
  28  |     await page.screenshot({ path: info.outputPath(`shell-${width}.png`), fullPage: false });
  29  |   }
  30  |   fs.writeFileSync(info.outputPath('shell-geometry.json'), JSON.stringify(findings, null, 2));
  31  |   console.log('SHELL_GEOMETRY', JSON.stringify(findings));
  32  | });
  33  | 
