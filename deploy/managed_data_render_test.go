...
func TestManagedDataModesFailClosed(t *testing.T) {
	for _, tc := range []struct {
		...
		{sets: []string{"postgres.mode=invalid"}, want: "postgres.mode must be"},
		{sets: []string{"redis.mode=sentinel", "redis.sentinel.passwordSecretRef.name=x", "redis.sentinel.replicas=2"}, want: "greater than or equal to 3"},
	} {
