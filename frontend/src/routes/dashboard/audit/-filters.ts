...
export function countAdvancedFilters(filters: AuditFilters): number {
  return ADVANCED_KEYS.filter((key) => isFilterActive(key, filters[key]))
    .length;
}
...
export function buildAuditQuery(
  ...
  return {
    ...
    action_class:
      filters.actionClass !== "all" ? filters.actionClass : undefined,
...
export function auditFilterChips(
  ...
  if (filters.audience === "system")
    chips.push({ key: "audience", label: "System activity" });
  if (filters.audience === "all")
    chips.push({ key: "audience", label: "All activity" });
  if (filters.actionClass !== "all")
    chips.push({ key: "actionClass", label: `Class: ${filters.actionClass}` });
  if (filters.result !== "all")
    chips.push({ key: "result", label: `Result: ${filters.result}` });
