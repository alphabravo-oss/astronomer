...
describe("cluster condition noise filter", () => {
  it("hides a capability the cluster simply does not have", () => {
    ...
    expect(
      isNoisyCapabilityCondition(cond("GatewayAPISupported", "False")),
    ).toBe(true);
    expect(
      isNoisyCapabilityCondition(cond("GatewayAPISupported", "Unknown")),
    ).toBe(true);
  });

  it("keeps a capability the cluster does have", () => {
    expect(
      isNoisyCapabilityCondition(cond("GatewayAPISupported", "True")),
    ).toBe(false);
  });
