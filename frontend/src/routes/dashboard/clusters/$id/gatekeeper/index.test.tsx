...
describe("ClusterGatekeeperPage", () => {
  ...
  it("disables Apply when the user lacks cluster write permission", () => {
    mockUseClustersUpdate.mockReturnValue({
      canWrite: false,
      reason: "You need clusters:update",
    });
    mockedList.mockReturnValue({
      data: [],
      isLoading: false,
  ...
  it("shows truthful pending deletion state and hides duplicate delete", () => {
    mockUseClustersUpdate.mockReturnValue({ canWrite: true, reason: "" });
    mockedList.mockReturnValue({
      data: [
        {
    ...
    expect(
      screen.queryByRole("button", { name: /delete constraint/i }),
