...
export function InstallChartModal({
  ...
  const form = useAppForm({
    defaultValues: {
      clusterId: allowedClusterIds.includes(presetClusterId)
        ? presetClusterId
        : "",
  ...
  const [editorMode, setEditorMode] = useState<"form" | "yaml">(
    schema ? "form" : "yaml",
  );
  ...
  const [schemaValues, setSchemaValues] = useState<HelmValuesObject>(() => {
    ...
    return (
      schema ? mergeSchemaDefaults(schema, parsed) : parsed
    ) as HelmValuesObject;
  });
  ...
  const handleYAMLChange = (nextYAML: string) => {
    ...
    if (parsed == null) {
      setYamlError(
        "YAML must parse to an object before the form can stay in sync.",
      );
      return;
    }
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <label
              className="text-sm font-medium text-foreground"
              htmlFor="field-60f181fe-195"
            >
              Values Override
            </label>
            <p className="text-xs text-muted-foreground mt-1">
              {schema
                ? "Edit with the chart schema form or switch to raw YAML."
                : "Raw YAML editor for chart values."}
            </p>
          </div>
          {schema && (
        ...
        {schema && editorMode === "form" ? (
          <div className="rounded-lg border border-border bg-muted/20 p-4">
            <HelmValuesForm
              schema={schema}
              value={schemaValues}
              onChange={handleSchemaValuesChange}
            />
          </div>
        ) : (
