 ...
 * from `GET /admin/quota-usage/`. Each row links to the detail page; threshold
...
import { useQuotaPlans, useQuotaUsage } from "@/components/settings/hooks";
import type { QuotaPlanView, QuotaUsageRow } from "@/lib/api/quotas";

function enforcementBadge(e: QuotaPlanView["enforcement"]) {
  const palette: Record<QuotaPlanView["enforcement"], string> = {
...
function QuotaPlansTable() {
  ...
  const columns: Column<QuotaPlanView>[] = [
    {
      key: "name",
      header: "Plan",
      accessor: (row) => (
    ...
    },
    {
      ...
      accessor: (row) => (
        <span className="text-xs text-muted-foreground tabular-nums">
          ...
          {row.maxNamespacesPerProject || "∞"} namespaces
        </span>
      ),
    },
  ];
    ...
    <SettingsAuthGate>
      <PageShell>
        ...
        <PageHeader
          ...
          title="Quota plans"
          description="Per-tenant caps on projects, clusters, namespaces, membership, API tokens, and streams."
          actions={
            <>
              <ActionButton
