    ...
    <PageShell>
      ...
      <PageHeader
        ...
        description={
          <>
            Profile{" "}
            <span className="font-mono text-foreground">{scan.scanType}</span>
            {scan.startedAt && (
              <> · Started {formatDate(scan.startedAt, "MMM d, HH:mm")}</>
            )}
            {scan.completedAt && (
              <> · Completed {formatDate(scan.completedAt, "MMM d, HH:mm")}</>
            )}
          </>
        }
        actions={
      ...
      {scan.errorMessage && (
        <div className="rounded-md border border-status-error/30 bg-status-error/5 p-3">
          ...
          <p className="text-xs text-status-error/80 mt-1 font-mono">
            {scan.errorMessage}
          </p>
        </div>
      )}
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {cells.map((c) => {
        const Icon = c.icon;
        return (
          <div key={c.label} className={cn("rounded-lg border p-4", c.bg)}>
...
function FindingsSection({
  ...
  const [severityFilter, setSeverityFilter] = useState<
    CISFindingSeverity | "all"
  >("all");
  const [statusFilter, setStatusFilter] = useState<CISFindingStatus | "all">(
    "all",
  );
...
function FilterPills<T extends string>({
  ...
  return (
    <div className="inline-flex items-center gap-1 rounded-md border border-border bg-card p-1">
      <span className="text-2xs text-muted-foreground px-1.5 hidden sm:inline">
        {label}:
      </span>
      {options.map((opt) => (
        <button
...
export const Route = createFileRoute("/dashboard/security/scans/$scanId/")({
