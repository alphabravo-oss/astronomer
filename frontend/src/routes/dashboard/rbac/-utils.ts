...
export function isUserLocked(
  user: Pick<User, "lockedUntil" | "locked_until">,
): boolean {
...
export function isValidNamespace(namespace: string): boolean {
  ...
  return (
    namespace.length <= 63 && /^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/.test(namespace)
  );
}
...
export function crdGrantCount(rules?: PolicyRule[]): number {
  return (rules ?? []).filter(
    (rule) => (rule.apiGroups ?? rule.api_groups ?? []).length > 0,
  ).length;
}
...
export function clusterLabel(
  cluster: Pick<Cluster, "displayName" | "name">,
): string {
  return cluster.displayName || cluster.name;
}

export function projectLabel(
  project: Pick<Project, "displayName" | "name">,
): string {
