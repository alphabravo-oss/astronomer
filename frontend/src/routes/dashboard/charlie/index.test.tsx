...
beforeAll(async () => {
  await (
    CharlieHub as typeof CharlieHub & { preload?: () => Promise<unknown> }
  ).preload?.();
});
...
describe("Charlie hub acceptance", () => {
  ...
  it("keeps private conversations separate from shared investigations", async () => {
    renderHub();
    expect(
      await screen.findByText("Private operator question"),
    ).toBeInTheDocument();
  ...
  it("uses URL-backed investigation filters and links only the originating product resource", async () => {
    search = new URLSearchParams(
      "tab=investigations&incident=incident-1&severity=high&cluster=agent-east&source=event&from=2026-08-01&to=2026-08-07",
    );
    renderHub();
    expect(
      await screen.findAllByText("Investigate flapping agent"),
    ).toHaveLength(2);
    expect(screen.getByLabelText("Investigation severity")).toHaveValue("high");
    expect(screen.getByLabelText("Agent connection record")).toHaveValue(
      "agent-east",
    );
    expect(screen.getByText("Repeated 3 times.")).toBeInTheDocument();
    expect(
      screen.getByRole("link", {
        name: /Open originating agent connection record/,
      }),
    ).toHaveAttribute("href", "/dashboard/agents?connection=agent-east");
  ...
  it("shows a blocked finding's exact reason and only its server-authorized lifecycle decisions", async () => {
    search = new URLSearchParams(
      "tab=findings&finding=finding-1&status=open&severity=high&source=cluster_agents&resource=agent-east&block=authority.read_only_write&from=2026-08-01&to=2026-08-07",
    );
    renderHub();
    expect(
      await screen.findByText("No action: authority.read_only_write"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/No Charlie action is authorized/),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: "start remediation" }),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(transitionCharlieFinding).toHaveBeenCalledWith(
        "finding-1",
        "acknowledge",
      ),
    );
  ...
  it("requires exact confirmation and preserves bounded rationale for one eligible approval", async () => {
    ...
    renderHub();
    expect(
      await screen.findByText("Restart one server replica"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("management_components:update"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Restart one unhealthy management-plane replica"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("One server replica reconnects"),
    ).toBeInTheDocument();
    expect(screen.getByText("Withheld by Charlie")).toBeInTheDocument();
    fireEvent.change(
      screen.getByLabelText("Rationale for Restart one server replica"),
      {
  ...
  it.each(["approved", "denied", "expired"] as const)(
    ...
    async (state) => {
      ...
      expect(
        screen.queryByRole("button", { name: "Review denial" }),
      ).toBeNull();
      expect(decideCharlieApproval).not.toHaveBeenCalled();
    },
  );
  ...
  it("removes every decision when live Charlie permission is absent", async () => {
    useAuthStore.setState({
      user: { id: "viewer" } as User,
      isAuthenticated: true,
    });
    search = new URLSearchParams("tab=findings&finding=finding-1");
    renderHub();
    expect(
      await screen.findByText(/Requires charlie:update/),
    ).toBeInTheDocument();
