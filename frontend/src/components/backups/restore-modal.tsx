...
export function RestoreModal({ backup, onClose }: RestoreModalProps) {
  ...
  const [includedFilter, setIncludedFilter] =
    useState<string[]>(sourceNamespaces);
  ...
  const namespacesOK =
    sourceNamespaces.length === 0 || includedFilter.length > 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const restore = await create.mutateAsync({
        ...
        included_namespaces:
          includedFilter.length === sourceNamespaces.length
            ? undefined
            : includedFilter,
        namespace_mapping:
          Object.keys(namespaceMapping).length > 0
            ? namespaceMapping
            : undefined,
    ...
    >
      ...
      {sourceNamespaces.length > 0 && (
        <div className="space-y-1.5">
          <span
            id="restore-namespaces-label"
            className="text-sm font-medium text-foreground"
          >
            Restore namespaces
          </span>
          <p className="text-xs text-muted-foreground">
            Pick a subset of the namespaces captured in this backup, or leave
            all selected to restore everything.
          </p>
          <div
            role="group"
            aria-labelledby="restore-namespaces-label"
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label
            className="text-sm font-medium text-foreground"
            htmlFor="field-9bca5ae6-153"
          >
            Namespace mapping (optional)
          </label>
          <button
            onClick={() =>
              setMappingRows((rs) => [...rs, { from: "", to: "" }])
            }
            type="button"
        ...
        {mappingRows.length === 0 && (
          <p className="text-xs text-muted-foreground">
            Use this to restore namespace{" "}
            <span className="font-mono">prod</span> as
        ...
        {mappingRows.map((row, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() =>
                setMappingRows((rs) => rs.filter((_, j) => j !== i))
              }
      ...
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-foreground">
          Type <span className="font-mono text-primary">{backup.name}</span> to
          confirm
        </label>
        <input
          type="text"
