...
export function renderPreviewValues(
  values: Record<string, unknown> | undefined,
): string {
  if (!values || Object.keys(values).length === 0)
    return "# The backend rendered no values.";
  try {
...
export function StackPreviewDialog({
  ...
  return (
    ...
    >
      {isLoading && (
        <p className="text-sm text-muted-foreground">Rendering chart values…</p>
      )}
        ...
        <div className="flex items-start gap-2 rounded-md border border-status-warning/30 bg-status-warning/10 px-3 py-2 text-xs text-status-warning">
          ...
          <div className="min-w-0">
            <p className="font-medium">
              This change cannot be applied in place.
            </p>
            <p className="mt-0.5">
              Upgrade will be rejected; use Replace, which uninstalls the
              release and reinstalls it.
              {preview.replaceReasons?.length
