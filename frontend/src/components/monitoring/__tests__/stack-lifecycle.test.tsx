...
vi.mock("@/lib/hooks", () => ({
  ...
  useFeatureFlags: vi.fn(() => ({
    data: { "feature.shared_grafana": true, "feature.hosted_loki": true },
  })),
}));
...
vi.mock("@/lib/api/monitoring-stack", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@/lib/api/monitoring-stack")>();
  return {
    ...actual,
...
interface FamilyCase {
  ...
  name: string;
  targetType: MonitoringOperation["targetType"];
  releaseName: string;
  namespace: string;
...
function statusPerTarget(
  byKey: Partial<Record<FamilyCase["key"], MonitoringStackStatusBase>>,
) {
...
describe.each(FAMILIES)("$name lifecycle screen", (family) => {
  ...
  it("previews the rendered Helm values without leaving the screen", async () => {
    ...
    expect(
      within(dialog).getByText("Rendered Helm values"),
    ).toBeInTheDocument();
  ...
  it("queues an install and shows it in flight instead of reporting success", async () => {
    ...
    await waitFor(() =>
      expect(
        within(panel).getByText("Install in progress"),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("requires a typed confirmation naming the release before uninstalling", async () => {
    ...
    const panel = await panelFor(family);
    const uninstall = await within(panel).findByRole("button", {
      name: "Uninstall",
    });
    ...
    expect(
      screen.getByText(
        new RegExp(`deletes the Helm release "${family.releaseName}"`),
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(new RegExp(`namespace ${family.namespace}`)),
    ).toBeInTheDocument();
    ...
    const confirmInput = screen
      .getAllByPlaceholderText(family.releaseName)
      .at(-1) as HTMLElement;
  ...
  it("guards Replace with the same typed confirmation as Uninstall", async () => {
    ...
    fireEvent.click(
      await within(panel).findByRole("button", { name: "Edit configuration" }),
    );
    fireEvent.click(
      await within(panel).findByRole("button", { name: "Replace" }),
    );
    ...
    fireEvent.change(confirmInput, { target: { value: family.releaseName } });
    fireEvent.click(
      screen.getAllByRole("button", { name: "Replace" }).at(-1) as HTMLElement,
    );
  ...
  it("hides every control a monitoring:read caller would be 403d on, keeping preview", async () => {
    ...
    expect(
      within(panel).queryByRole("button", { name: "Install" }),
    ).not.toBeInTheDocument();
    expect(
      within(panel).queryByRole("button", { name: "Upgrade" }),
    ).not.toBeInTheDocument();
    expect(
      within(panel).queryByRole("button", { name: "Replace" }),
    ).not.toBeInTheDocument();
    expect(
      within(panel).queryByRole("button", { name: "Uninstall" }),
    ).not.toBeInTheDocument();
    // The desired-configuration form is a mutation affordance too.
    expect(
      within(panel).queryByText("Desired configuration"),
    ).not.toBeInTheDocument();
    // Preview is monitoring:read on the backend, so it stays.
    expect(
      within(panel).getByRole("button", { name: "Preview" }),
    ).toBeInTheDocument();
  });
});
...
describe("hosted Loki feature flag", () => {
  it("hides the Loki panel unless feature.hosted_loki is exactly true", async () => {
    grant(["read", "update"]);
    vi.mocked(useFeatureFlags).mockReturnValue({
      data: { "feature.shared_grafana": true },
    } as never);
...
describe("per-cluster monitoring stack page", () => {
  it("honours a monitoring grant scoped to this cluster exactly like the API", async () => {
    ...
    expect(
      within(panel).getByRole("button", { name: /^Set up / }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Monitoring access required"),
    ).not.toBeInTheDocument();
  });
  ...
  it("prefills storageConfigId from a healthy shared Thanos stack", async () => {
    ...
    expect(
      await screen.findByTestId("shared-thanos-bucket-prefill"),
    ).toHaveTextContent("Use shared Thanos bucket");
...
describe("shared monitoring stacks page", () => {
  it("shows Open shared Grafana only when authMode is proxy", async () => {
    ...
    const clusteripPanel = await panelFor(
      FAMILIES.find((f) => f.key === "grafana")!,
    );
    await waitFor(() =>
      expect(within(clusteripPanel).getByText("Healthy")).toBeInTheDocument(),
    );
    expect(
      within(clusteripPanel).queryByRole("link", {
        name: "Open shared Grafana",
      }),
    ).not.toBeInTheDocument();
    unmount();
    ...
    render(<SharedMonitoringStacksPage />, { wrapper: Wrapper });
    const proxyPanel = await panelFor(
      FAMILIES.find((f) => f.key === "grafana")!,
    );
    await waitFor(() =>
      expect(within(proxyPanel).getByText("Healthy")).toBeInTheDocument(),
    );
  ...
  it("renders Thanos and Alertmanager as independent panels", async () => {
    ...
    await waitFor(() =>
      expect(within(thanos).getByText("Healthy")).toBeInTheDocument(),
    );
    ...
    expect(
      within(alertmanager).getByRole("button", { name: /^Set up / }),
    ).toBeInTheDocument();
    expect(
      within(thanos).queryByRole("button", { name: /^Set up / }),
    ).not.toBeInTheDocument();
    expect(
      within(thanos).getByRole("button", { name: "Edit configuration" }),
    ).toBeInTheDocument();
  });
  ...
  it("shows the whole page as permission-blocked without monitoring:read", async () => {
    ...
    expect(
      await screen.findByText("Monitoring access required"),
    ).toBeInTheDocument();
