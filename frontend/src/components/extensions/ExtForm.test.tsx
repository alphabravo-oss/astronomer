...
describe("buildSubmitBody", () => {
  it("coerces number/toggle/text and drops blank numbers", () => {
    expect(
      buildSubmitBody(inputs, { title: "hi", count: "5", enabled: true }),
    ).toEqual({
      title: "hi",
      count: 5,
      enabled: true,
    ...
    expect(
      buildSubmitBody(inputs, { title: "hi", count: "", enabled: false }),
    ).toEqual({
      title: "hi",
      enabled: false,
    });
...
describe("missingRequired", () => {
  it("flags an empty required text field", () => {
    expect(
      missingRequired(inputs, { title: "", count: "", enabled: false }),
    ).toEqual(["title"]);
  });

  it("passes when required fields are filled", () => {
    expect(
      missingRequired(inputs, { title: "ok", count: "", enabled: false }),
    ).toEqual([]);
  });

  it("treats a required toggle as missing until checked", () => {
    const req: FormInput[] = [
      { name: "agree", label: "Agree", type: "toggle", required: true },
    ];
