...
describe("SandboxedExtension — inbound origin checks", () => {
  it("ignores a message from a foreign origin (spoofing) — no proxy call, no reply", async () => {
    ...
    dispatchMessage(
      bridge("ext/data.request", {
        id: "r2",
        payload: { dataSource: "podCost" },
      }),
      "https://evil.example",
      win,
    );
...
describe("SandboxedExtension — data request/response brokering", () => {
  it("brokers an allowed ext/data.request through the proxy and replies correlated by id", async () => {
    const res: ExtensionDataResponse = {
      data: { rows: [{ ns: "a" }] },
      shape: "list",
      meta: { dataSourceId: "podCost" },
    };
    ...
    dispatchMessage(
      bridge("ext/data.request", {
        id: "r2",
        payload: { dataSource: "podCost", query: { window: "7d" } },
      }),
      ORIGIN,
      win,
    );
...
describe("SandboxedExtension — handshake rejection", () => {
  it("renders the incompatible placeholder when ext/ready manifestSha mismatches", async () => {
    const { container } = render(
      <SandboxedExtension mount={mount()} manifestSha="sha256:installed" />,
    );
    ...
    dispatchMessage(
      bridge("ext/ready", {
        payload: { manifestSha: "sha256:different", acceptsProtocol: [1] },
      }),
      ORIGIN,
      win,
    );
    ...
    await waitFor(() =>
      expect(
        container.querySelector('[data-bridge-state="incompatible"]'),
      ).toBeInTheDocument(),
    );
  });
});
