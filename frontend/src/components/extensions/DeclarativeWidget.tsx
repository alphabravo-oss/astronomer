...
function FetchingWidget({
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () =>
      fetchExtensionData(extensionName, spec.dataSource, { context }),
    staleTime: 30_000,
  });
  ...
  switch (spec.kind) {
    case "table": {
      ...
      return (
        <ExtTable rows={rows} fields={spec.fields} emptyText={spec.emptyText} />
      );
    }
    case "chart": {
      if (!spec.chart) {
        return (
          <ErrorState title="Invalid widget" description="chart spec missing" />
        );
      }
      const rows = extractRows(data);
      return (
        <ExtChart rows={rows} spec={spec.chart} emptyText={spec.emptyText} />
      );
    }
    case "stat": {
      if (!spec.stat) {
        return (
          <ErrorState title="Invalid widget" description="stat spec missing" />
        );
      }
