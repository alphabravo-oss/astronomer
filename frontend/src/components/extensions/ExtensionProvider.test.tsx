...
function response(
  over: Partial<ExtensionMountsResponse> = {},
): ExtensionMountsResponse {
  return {
    sidebar: [],
    dashboardWidgets: [],
...
describe("ExtensionProvider runtime", () => {
  ...
  it("exposes the indexed registry to useExtensionMounts(point)", async () => {
    mockedMounts.mockResolvedValue(
      response({ clusterTabs: [tab("a"), tab("b")] }),
    );
  ...
  it("returns an empty list for a point with no mounts", async () => {
    ...
    await waitFor(() =>
      expect(extensionsApi.getExtensionMounts).toHaveBeenCalled(),
    );
  ...
  it("passes the host theme tokens through for the Tier-2 handshake", async () => {
    ...
    const { result } = renderHook(() => useExtensionTheme(), {
