...
vi.mock("@/lib/api/kubernetes-proxy", () => ({ k8sGet: vi.fn() }));
...
it("maps served CRDs into groups and caches the single discovery request", async () => {
  vi.mocked(k8sGet).mockResolvedValue({
    items: [
      crd("cert-manager.io", "certificates", "Certificate"),
      crd("cert-manager.io", "issuers", "Issuer"),
      crd("monitoring.coreos.com", "prometheuses", "Prometheus"),
    ],
  });
...
it("distinguishes failed discovery from a cluster without CRDs", async () => {
  vi.mocked(k8sGet).mockRejectedValue(new Error("Forbidden"));
  const { result } = renderHook(() => useClusterDiscovery("c-1"), { wrapper });
