...
export function ResourceActions({
  ...
  const scalePerm = useClusterResourcePermission(
    clusterId,
    resourceType,
    "scale",
    permissionResource,
  );
  const restartPerm = useClusterResourcePermission(
    clusterId,
    resourceType,
    "restart",
    permissionResource,
  );
  const updatePerm = useClusterResourcePermission(
    clusterId,
    resourceType,
    "update",
    permissionResource,
  );
  const deletePerm = useClusterResourcePermission(
    clusterId,
    resourceType,
    "delete",
    permissionResource,
      ...
      {WORKLOAD_RESTARTABLE_KINDS.includes(kind) && (
        ...
        >
          <RotateCw
            className={cn(
              "h-3.5 w-3.5",
              restartWorkload.isPending && "animate-spin",
            )}
          />{" "}
          Restart
        </button>
