...
describe("DataTable behavior (TanStack Table engine)", () => {
  ...
  it("persists column visibility to localStorage when persistKey is set", () => {
    ...
    expect(
      JSON.parse(window.localStorage.getItem("dt:test-table:visibility")!),
    ).toMatchObject({
      size: false,
    });
  ...
  it("renders drag resize handles only when resizable is enabled", () => {
    ...
    expect(container.querySelectorAll("[data-resize-handle]")).toHaveLength(
      columns.length,
    );
  });
    ...
    const stubLayout = () => {
      ...
      const descriptors = props.map(
        (p) =>
          [
            p,
            Object.getOwnPropertyDescriptor(HTMLElement.prototype, p),
          ] as const,
      );
    ...
    it("moves row focus with ArrowDown via roving tabindex", async () => {
      ...
      try {
        ...
        const rowAt = (i: number) =>
          screen
            .getByRole("grid")
            .querySelector<HTMLElement>(`[data-row-index="${i}"]`)!;
