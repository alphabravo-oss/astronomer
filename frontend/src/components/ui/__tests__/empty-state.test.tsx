...
describe("EmptyState", () => {
  it("renders title and description", () => {
    ...
    expect(
      screen.getByText("This surface is gated to platform administrators."),
    ).toBeInTheDocument();
  });
  ...
  it("renders a shared loading state", () => {
    render(
      <LoadingState
        ...
        description="Fetching current consumption."
      />,
    );
  ...
  it.each([
    [PartialState, "Partial results", "Retry missing data"],
    [OfflineState, "Connection unavailable", "Reconnect"],
    [StaleState, "Showing stale data", "Refresh now"],
    [TerminalFailureState, "Operation failed", "Try again"],
  ] as const)(
    "renders an actionable %s",
    (Component, expectedTitle, expectedAction) => {
      const onRetry = vi.fn();
      render(<Component onRetry={onRetry} onRefresh={onRetry} />);
  ...
  it("announces retrying work without offering a duplicate action", () => {
    render(<RetryingState />);
