...
function eventStartedInRowAction(event: React.MouseEvent<HTMLElement>) {
  return (
...
export function DataTable<T>({
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(
    () =>
      Object.fromEntries(
        columns.filter((c) => c.hidden).map((c) => [c.key, false]),
      ),
  );
  ...
  const table = useReactTable<T>({
    ...
    state: {
      ...
      ...(effectiveServerSide
        ? { pagination: effectiveServerSide.pagination }
        : {}),
    },
    ...(resizable
      ? {
    ...
    onColumnVisibilityChange: (updater: Updater<VisibilityState>) =>
      setColumnVisibility((prev) => {
        ...
        if (persistKey && typeof window !== "undefined") {
          try {
            window.localStorage.setItem(
              visibilityStorageKey(persistKey),
              JSON.stringify(next),
            );
          } catch {
            /* ignore quota/availability errors — persistence is best-effort */
    ...
    <div className={cn("space-y-3", className)}>
              ...
              <TableHeader>
                <TableRow className="border-b border-border bg-muted/50">
                  ...
                  {activeColumns.map((col) => {
                    ...
                    const header = resizable
                      ? headerByKey.get(col.key)
                      : undefined;
                    return (
                      <TableHead
                        key={col.key}
      ...
      {!virtualized && totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">
            Showing {totalRows === 0 ? 0 : page * effPageSize + 1}-
            {page * effPageSize + rows.length} of {totalRows.toLocaleString()}
          </span>
