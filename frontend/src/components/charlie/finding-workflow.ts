...
export function findingWorkflowGuidance(finding: CharlieFinding): string {
  switch (finding.workflowState) {
    case "approval_pending":
      return "This action requires human approval. Open the separate approvals list to review an exact pending approval; this finding cannot approve or retry it.";
    case "manual_remediation_required":
      return "Follow the bounded manual checks or start manual remediation. No Charlie action is authorized by this finding.";
