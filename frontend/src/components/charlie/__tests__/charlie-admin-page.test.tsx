...
const feature = vi.hoisted(() => ({
  value: {
    data: { "feature.charlie": true } as
      { "feature.charlie": boolean } | undefined,
    isError: false,
    refetch: vi.fn(),
  },
}));
...
beforeEach(() => {
  ...
  api.getCharlieAgent.mockResolvedValue({
    ...
    replicas: [
      ...
      },
      {
        ...
        role: "standby",
        state: "ready",
        lastHeartbeatAt: "2026-08-05T09:59:59Z",
        version: "1.0.0",
      },
    ],
    fencingEpoch: 9,
  ...
  api.getCharlieKubernetesVisibility.mockResolvedValue({
    ...
    scopeSummary:
      "Product-owned management resources plus bounded cluster diagnostics; downstream clusters excluded",
  });
  ...
  api.updateCharlieAlertPolicy.mockImplementation(async (policy) => ({
    ...policy,
...
describe("Charlie administration acceptance", () => {
  it("configures Kubernetes observation independently while keeping hard prohibitions visible", async () => {
    ...
    expect(
      await screen.findByText("Kubernetes API visibility"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Downstream clusters, Secret values, exec, attach/i),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("radio", { name: "Product namespace" }));
    fireEvent.click(
      screen.getByRole("button", { name: "Save visibility policy" }),
    );
    await waitFor(() =>
      expect(api.updateCharlieKubernetesVisibility).toHaveBeenCalledWith({
        profile: "product_namespace",
        podLogs: false,
        revision: 4,
      }),
  ...
  it("configures product-owned alert routing without implying action authority", async () => {
    ...
    expect(
      await screen.findByText("Actionable finding alerts"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/cannot approve, authorize, or dispatch work/i),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("checkbox", {
        name: "Route Charlie alerts to Platform on-call",
      }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Save alert policy" }));
    await waitFor(() =>
      expect(api.updateCharlieAlertPolicy).toHaveBeenCalledTimes(1),
    );
      ...
      automationGrants: [
        {
          ...
          scope: "global",
          source: "global_role:Charlie Automation",
        },
      ],
    });
    renderWithClient(<AccessTab />);
    expect(
      await screen.findByText("Your Charlie permissions"),
    ).toBeInTheDocument();
  ...
  it("covers disabled, loading, denied, and authorized feature/permission states", async () => {
    ...
    expect(
      await screen.findByText("Independent diagnostic checks"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Charlie administration restricted"),
    ).toBeInTheDocument();
    denied.unmount();
  ...
  it("requires a typed disconnect confirmation before deactivating Charlie", async () => {
    ...
    expect(
      screen.getByText(
        /Charlie navigation, findings, sessions, and the product agent stop/i,
      ),
    ).toBeInTheDocument();
  ...
  it("hides connect until the current Charlie connection is disconnected", async () => {
    ...
    expect(await screen.findByText("Charlie is connected")).toBeInTheDocument();
    expect(
      screen.getByText("https://charlie.example.test"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: "Validate connection" }),
    ).toBeNull();
  });
  ...
  it("connects Charlie with endpoint and connect token", async () => {
    ...
    api.consumeCharlieConnect.mockResolvedValue({
      ...safeReview,
      state: "consumed",
    });
    renderWithClient(<ConnectionTab />);
  ...
  it("renders only verified safe onboarding metadata and never package credentials", async () => {
    ...
    const validate = await screen.findByRole("button", {
      name: "Validate signature locally",
    });
    await waitFor(() => expect(validate).toBeEnabled());
    fireEvent.click(validate);
    await waitFor(() =>
      expect(api.validateCharlieOnboarding).toHaveBeenCalledTimes(1),
    );
    expect(await screen.findByText("Validated package")).toBeInTheDocument();
    expect(screen.getByText("charlie/v1")).toBeInTheDocument();
    expect(
      screen.getAllByText(safeReview.signingFingerprint).length,
    ).toBeGreaterThan(0);
    for (const canary of [
      secret,
      artifactCredential,
  ...
  it("shows truthful per-replica state and routes lifecycle changes through Flux delivery", async () => {
    ...
    expect((await screen.findAllByText("instance-0")).length).toBeGreaterThan(
      0,
    );
    expect(screen.getAllByText("instance-1").length).toBeGreaterThan(0);
    expect(
      screen.getByText(/installed from Charlie when you connect/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", {
        name: /install|upgrade|rollback|rotate|uninstall/i,
      }),
    ).toBeNull();
  });
  ...
  it("keeps missing-agent state read-only instead of calling deleted lifecycle routes", async () => {
    ...
    renderWithClient(<AgentTab />);
    expect(
      await screen.findByText(/installed from Charlie when you connect/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", {
        name: /install|upgrade|rollback|rotate|uninstall/i,
  ...
  it("keeps Charlie-owned intelligence configuration out of Astronomer", async () => {
    ...
    expect(
      await screen.findByText(
        /Model providers, knowledge packs, and routes stay in Charlie/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/not a durable Charlie API key/i),
    ).toBeInTheDocument();
  });
  ...
  it("accepts a rediscovered catalog on Astronomer so mode can be raised", async () => {
    ...
    api.getCharlieKubernetesVisibility.mockResolvedValue({
      ...
      scopeSummary:
        "Product-owned resources in the Astronomer management namespace",
    });
    ...
    expect(
      await screen.findByRole("button", {
        name: "Accept rediscovered catalog",
      }),
    ).toBeEnabled();
    expect(screen.getByRole("button", { name: /^read only/i })).toBeDisabled();
    fireEvent.click(
      screen.getByRole("button", { name: "Accept rediscovered catalog" }),
    );
    await waitFor(() =>
  ...
  it("explains every authority mode and blocks elevation until disclosure acknowledgement", async () => {
    renderWithClient(<ModeTab />);
    expect(
      await screen.findByText("Every write requires exact approval"),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /^read only/i })).toBeDisabled();
    expect(
      screen.getByRole("button", { name: /^approval required/i }),
    ).toBeDisabled();
    ...
    expect(
      screen.getByRole("button", { name: "Emergency Disable" }),
    ).toBeEnabled();
    expect(screen.getByText(/Accept the rediscovered catalog/)).toHaveAttribute(
      "role",
      "status",
    );
    expect(
  ...
  it("uses product mode labels while sending stable wire values", async () => {
    ...
    renderWithClient(<ModeTab />);
    fireEvent.click(
      await screen.findByRole("button", { name: /^automation/i }),
    );
    expect(
      screen.getAllByText(/Includes Read only and Approval required/i).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByTestId("charlie-mode-confirm-allowed"),
    ).toHaveTextContent(/What Automation allows/i);
    ...
    expect(
      await screen.findByTestId("charlie-mode-transition"),
  ...
  it("does not call a pending Charlie confirmation an unverified agent ceiling", async () => {
    ...
    expect(
      screen.getByText(/Both product-agent replicas already report Read only/),
    ).toBeInTheDocument();
  ...
  it("shows validating state until agent ceiling is ready after mode change", async () => {
    ...
    expect(
      await screen.findByTestId("charlie-mode-transition"),
    ).toHaveAttribute("data-phase", "verifying");
  ...
  it("renders trigger rules when Charlie returns null scopes", async () => {
    ...
    expect(
      screen.queryByText(/Automation configuration unavailable/i),
    ).toBeNull();
  });
  ...
  it("exposes all automation policy fields and validates new rules before save", async () => {
    renderWithClient(<AutomationTab />);
    expect(
      await screen.findByText("astronomer.queue.retry_task"),
    ).toBeInTheDocument();
    expect(screen.getByText("exact session resource ID")).toBeInTheDocument();
    expect(
      screen.getByText(/task reaches a terminal state/),
    ).toBeInTheDocument();
    ...
    expect(screen.getByLabelText("Severities (comma separated)")).toHaveValue(
      "high",
    );
    expect(
      screen.getByLabelText("Cluster coverage threshold %"),
  ...
  it("fails closed when central action-policy verification is unavailable", async () => {
    ...
    renderWithClient(<AutomationTab />);
    expect(
      await screen.findByLabelText("Enable astronomer.queue.retry_task"),
    ).toBeDisabled();
    expect(
      screen.getByText(/until Charlie central is verified/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows action-policy conflicts without claiming the change was saved", async () => {
    api.updateCharlieActionPolicy.mockRejectedValue(
      new Error(
        "This action policy conflicts with current central allowlisting or bounded budget rules.",
      ),
    );
    renderWithClient(<AutomationTab />);
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      /conflicts with current central allowlisting/i,
    );
  });
  ...
  it("shows only bounded dead-letter metadata and requires typed retry confirmation", async () => {
    ...
    api.retryCharlieTriggerEvent.mockResolvedValue({
      id: "retry-a",
      state: "retry",
    });
    ...
    expect(view.container.textContent).not.toContain(
      "dead-letter-payload-dom-canary",
    );
  ...
  it("renders every independent diagnostic without making it core readiness", async () => {
    ...
    expect(
      await screen.findByText("Local database and configuration"),
    ).toBeInTheDocument();
    expect(screen.getByText("Agent standby replica")).toBeInTheDocument();
    expect(
      screen.getByText(/never participates in Astronomer's core readiness/i),
    ).toBeInTheDocument();
    expect(screen.getByText("correlation-a")).toBeInTheDocument();
    expect(
      screen.getByText(/Next action: Rotate the product bridge certificate/i),
    ).toBeInTheDocument();
  });
});
