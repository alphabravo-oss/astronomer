...
const commands: CharlieCommandDescriptor[] = [
  ...
  },
  {
    ...
    name: "investigate",
    label: "Investigate",
    description: "Investigate a subject",
...
describe("Charlie command composer helpers", () => {
  it("matches canonical names and aliases without treating prose as commands", () => {
    expect(commandSuggestions("/hea", commands).map((item) => item.id)).toEqual(
      ["health"],
    );
    expect(parseCharlieCommand("/system-health", commands)?.request).toEqual({
      id: "health",
      version: "1",
      arguments: {},
    });
    expect(
      parseCharlieCommand("please check health", commands),
    ).toBeUndefined();
  });
  ...
  it("requires and bounds command shape before building the server request", () => {
    ...
    expect(
      parseCharlieCommand("/investigate queue failures", commands)?.request
        .arguments,
    ).toEqual({ subject: "queue failures" });
    expect(
      parseCharlieCommand(`/investigate ${"x".repeat(513)}`, commands),
    ).toBeUndefined();
