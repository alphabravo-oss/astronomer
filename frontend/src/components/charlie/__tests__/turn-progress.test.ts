...
describe("Charlie turn progress", () => {
  it("extracts the bounded turn identity from the public event envelope", () => {
    expect(
      charlieProgressEventTurnId(event("turn.started", {}, "1").data),
    ).toBe("turn-1");
    expect(
      charlieProgressEventTurnId(JSON.stringify({ turn_id: "<unsafe>" })),
    ).toBeUndefined();
  });

  it("shows real tool lifecycle and live updates without retaining content", () => {
    let progress = initialCharlieTurnProgress(1000);
    progress = updateCharlieTurnProgress(
      progress,
  ...
  it("ignores replayed event IDs and unsafe capability labels", () => {
    ...
    const once = updateCharlieTurnProgress(
      initial,
      event(
        "tool.running",
        {
    ...
    const replay = updateCharlieTurnProgress(
      once,
      event("text.delta", { text: "duplicate" }, "2"),
      1300,
    );
  ...
  it("tracks failed and policy-blocked tools as distinct terminal outcomes", () => {
    ...
    expect(progress.completedToolCallIds).toEqual([
      "call-failed",
      "call-blocked",
    ]);
