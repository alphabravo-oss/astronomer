 ...
 * with a sibling `__<name>_set: true` flag preserved from the wire. We render
 * the placeholder
...
export function ConnectorForm({
  ...
  const form = useAppForm({
    ...
    onSubmit: ({ value }) => {
      ...
      // strip the raw `__<name>_set` marker the API echoed back at us so it
      // cannot leak back as a bogus config key.
        ...
        >
          ...
          {nested.keys.map((key) => {
            ...
            const fmeta: FieldMeta =
              meta.fields?.[lookupKey] ?? meta.fields?.[key] ?? {};
            return (
              <form.AppField
                key={key}
...
function NestedGroup({
  ...
  return (
    <div className="rounded-lg border border-border">
      ...
      <div
        className={cn(
          "p-4 space-y-4 border-t border-border",
          !open && "hidden",
        )}
      >
        {children}
