...
// The shared transport preserves the backend's stored-secret marker.
...
describe("ConnectorForm secret marker handling", () => {
  it("renders the stored-secret placeholder/helper from the wire marker", () => {
    render(
      <ConnectorForm
        spec={spec}
        initial={editInitial()}
    ...
    expect(
      screen.getByText("Stored — type a new value to rotate"),
    ).toBeInTheDocument();
  });
  ...
  it("submits without re-entering the secret and strips the marker from config", async () => {
    ...
    // ... and the wire marker is not persisted as a garbage config key.
    expect(body.config).not.toHaveProperty("__clientSecret_set");
...
describe("ConnectorForm validators (ported 1:1 from the imperative checks)", () => {
  ...
  it("rejects invalid connector names on create", async () => {
    const onSubmit = vi.fn();
    render(
      <ConnectorForm spec={spec} submitLabel="Create" onSubmit={onSubmit} />,
    );
