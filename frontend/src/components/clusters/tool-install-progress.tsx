...
export function ToolInstallProgress({
  operationId,
  toolName,
  onClose,
      ...
      <div className="mx-auto w-[80%]">
        <header className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            >
              <StatusIcon
                className={`h-3.5 w-3.5 ${!isTerminal ? "animate-spin" : ""}`}
              />
              {statusLabel}
            </span>
          </div>
        ...
        >
          {events.length === 0 && (
            <div className="text-muted-foreground flex items-center gap-2">
              <Loader2 className="h-3 w-3 animate-spin" /> Waiting for the
              operation to start…
            </div>
          )}
          {events.map((ev) => (
          ...
          {failed && op?.errorMessage && (
            <div className="mt-2 text-status-error whitespace-pre-wrap">
              {op.errorMessage}
            </div>
          )}
        </div>
