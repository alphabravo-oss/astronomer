...
describe("isStoredSecret", () => {
  it("detects the exact stored-secret wire marker", () => {
    expect(isStoredSecret({ __clientSecret_set: true }, "clientSecret")).toBe(
      true,
    );
...
describe("stripUntouchedSecrets (pure)", () => {
  const dirtyForm = (dirty: Record<string, boolean>) => ({
    getFieldMeta: (name: never) => ({
      isDirty: dirty[name as string] ?? false,
    }),
  });

  it("drops pristine secrets, keeps dirty ones, and strips wire markers", () => {
    const value = {
      clientId: "abc",
      clientSecret: "typed",
      bindPW: "",
    ...
    >
      <form.AppField name="config.clientSecret">
        {(field) => (
          <field.SecretField
            label="Client secret"
            stored={isStoredSecret(initial, "clientSecret")}
          />
        )}
      </form.AppField>
...
function markerInitial() {
  // Exactly what the explicit connector mapper hands an edit page.
  return {
    clientId: "abc",
    clientSecret: "",
    __clientSecret_set: true,
  };
}
...
describe("SecretField + stripUntouchedSecrets (marker variant)", () => {
  it("renders the stored placeholder + rotate hint while pristine and omits the secret on submit", async () => {
    ...
    await submitForm();
    // Untouched secret dropped (preserve-on-empty) and the wire marker
  ...
  it("D14: isDirty does not survive form.reset(refreshedInitial) — the secret is pristine (omitted) again", async () => {
    ...
    expect(
      formRef.current!.getFieldMeta("config.clientSecret" as never)?.isDirty,
    ).toBe(true);
    ...
    >
      <form.AppField name="password">
        {(field) => (
          <field.SecretField
            label="Password"
            stored={field.state.value === SENTINEL}
          />
        )}
      </form.AppField>
      <button type="submit">Save</button>
