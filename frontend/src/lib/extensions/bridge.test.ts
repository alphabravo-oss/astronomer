...
describe("validateInbound — origin/source/discriminator gate", () => {
    ...
    const r = validateInbound(
      {
        source: IFRAME_WIN,
        origin: ORIGIN,
        data: { v: 1, type: "ext/ready", ext: EXT, mount: MOUNT },
      },
      guard(),
    );
    expect(r.ok).toBe(false);
  ...
  it("drops a non-object payload", () => {
    const r = validateInbound(
      { source: IFRAME_WIN, origin: ORIGIN, data: "hello" },
      guard(),
    );
    expect(r.ok).toBe(false);
    expect(r.reason).toBe("not-bridge");
    ...
    const r = validateInbound(
      {
        source: IFRAME_WIN,
        origin: ORIGIN,
        data: bridgeMsg({ id: 5 as unknown as string }),
      },
      guard(),
    );
    expect(r.ok).toBe(false);
...
describe("validateNavigation — /dashboard/ allowlist", () => {
  ...
  it("rejects an absolute URL with a scheme", () => {
    expect(validateNavigation({ to: "https://evil.example" }).reason).toBe(
      "has-scheme",
    );
    expect(validateNavigation({ to: "javascript:alert(1)" }).reason).toBe(
      "has-scheme",
    );
  });

  it("rejects a protocol-relative URL", () => {
    expect(validateNavigation({ to: "//evil.example/dashboard/" }).reason).toBe(
      "protocol-relative",
    );
  });

  it("rejects path traversal", () => {
    expect(validateNavigation({ to: "/dashboard/../admin" }).reason).toBe(
      "traversal",
    );
  });

  it("rejects when a referenced placeholder has no supplied value", () => {
    expect(validateNavigation({ to: "/dashboard/clusters/{id}" }).reason).toBe(
      "unknown-placeholder",
    );
  });
});
...
describe("host -> iframe builders", () => {
  it("buildHello carries the handshake allowlist as ids only (no upstream paths)", () => {
    ...
    expect(JSON.stringify(msg.payload.dataSources)).not.toMatch(
      /path|rbac|proxy/,
    );
  ...
  it("buildDataResponse correlates the response to the request id (ok + deny)", () => {
    const ok = buildDataResponse(EXT, MOUNT, "r2", {
      ok: true,
      data: { rows: [] },
      shape: "list",
    });
