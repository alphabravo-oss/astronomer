...
function fakeProxyStream() {
  ...
  return {
    ...
    push(verb: string, o: unknown) {
      deliver({
        value: encoder.encode(JSON.stringify({ type: verb, object: o }) + "\n"),
        done: false,
      });
    },
    end() {
...
describe("identityOf", () => {
  it("prefers uid, falls back to namespace/name", () => {
    expect(
      identityOf({ metadata: { uid: "u1", name: "x", namespace: "ns" } }),
    ).toBe("u1");
    expect(identityOf({ metadata: { name: "x", namespace: "ns" } })).toBe(
      "ns/x",
    );
  });
});
...
describe("k8sCollection — proxy NDJSON source", () => {
  ...
  it("caches handles per (cluster, source)", () => {
    const a = k8sCollection({
      clusterId: "cache",
      source: { kind: "proxy", path: "apis/apps/v1/deployments" },
    });
    const b = k8sCollection({
      clusterId: "cache",
      source: { kind: "proxy", path: "apis/apps/v1/deployments" },
    });
    const c = k8sCollection({
      clusterId: "cache",
  ...
  it("seeds from the REST list then folds ADDED/MODIFIED/DELETED frames in order", async () => {
    ...
    global.fetch = vi
      .fn()
      .mockResolvedValue(stream.response) as unknown as typeof fetch;
    ...
    expect(calledUrl).toContain(
      "/clusters/c-fold/k8s/apis/apps/v1/deployments?watch=true",
    );
...
describe("podRowFromRaw", () => {
  it("shapes a raw pod into the display row (ready/restarts/status per container)", () => {
    ...
    expect(row.conditions[0]).toMatchObject({
      type: "Ready",
      status: "True",
