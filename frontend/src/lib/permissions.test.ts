...
describe("permissions", () => {
  ...
  it("filters cluster-scoped bindings by cluster id", () => {
    ...
    expect(
      can(user, "workloads", "read", { type: "cluster", id: "cluster-a" }),
    ).toBe(true);
    expect(
      can(user, "workloads", "read", { type: "cluster", id: "cluster-b" }),
    ).toBe(false);
  });

  it("explains denied scoped permissions with the required grant and request path", () => {
    const decision = explainPermission(baseUser, "clusters", "update", {
      type: "cluster",
      id: "cluster-a",
    });
    ...
    expect(decision.disabledReason).toContain(
      "Requires clusters:update for cluster cluster-a",
    );
    expect(decision.disabledReason).toContain(
      "Request access from a cluster owner or platform administrator.",
    );
  });
  ...
  it("reports the role binding that grants a scoped permission", () => {
    ...
    const decision = explainPermission(user, "clusters", "update", {
      type: "cluster",
      id: "cluster-a",
    });

    expect(decision.allowed).toBe(true);
    expect(decision.grantedBy).toEqual([
      "Cluster Operator (cluster:cluster-a)",
    ]);
    expect(decision.reason).toBe(
      "Granted by Cluster Operator (cluster:cluster-a).",
    );
  });
});
