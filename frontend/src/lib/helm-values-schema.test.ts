...
describe("helm-values-schema helpers", () => {
  ...
  it("parses and serializes YAML values", () => {
    const parsed = parseHelmValuesYAML(
      "replicaCount: 3\ningress:\n  enabled: true\n",
    );
    expect(parsed).toEqual({ replicaCount: 3, ingress: { enabled: true } });

    const dumped = dumpHelmValuesYAML({
      replicaCount: 3,
  ...
  it("appends and removes array items", () => {
    ...
    expect(getValueAtPath(appended, ["ingress", "hosts"])).toEqual([
      "example.com",
      "",
    ]);
...
describe("resolveSchemaRefs", () => {
  it("inlines a top-level $ref into $defs (cert-manager shape)", () => {
    ...
    expect(
      (out.properties!.image.properties as Record<string, { type: string }>).tag
        .type,
    ).toBe("string");
