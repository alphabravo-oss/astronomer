...
describe("formatToastError()", () => {
  ...
  it("uses plain Error messages", () => {
    expect(
      formatToastError("Delete failed", new Error("permission denied")),
  ...
  it("falls back for unknown shapes", () => {
    expect(
      formatToastError("Apply failed", { code: "unknown" }, "try again"),
    ).toBe("Apply failed: try again");
  });
});
