...
describe("cluster-scoped application catalog", () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.restoreAllMocks();
  });

  it("scopes chart browse and version requests to the selected cluster", async () => {
    ...
    await getHelmCharts({ clusterId: "cluster-1", search: "metrics" });
    await getHelmChartVersions("cluster-1", "chart-1");

    expect(generated.getCatalogCharts).toHaveBeenCalledWith({
      query: { cluster_id: "cluster-1", project_id: undefined, limit: 200 },
    });
  ...
  it("keeps cluster browse helpers cluster-scoped", async () => {
    vi.mocked(generated.getCatalogCharts).mockResolvedValueOnce({
    ...
    await listCatalogCharts({ clusterId: "cluster-1", limit: 60 });
    await listRecommendedCharts("project-1", 12);
    await listChartVersions("cluster-1", "chart-1");
    await getChartDefaultValues("cluster-1", "chart-1", "1.2.3");

    expect(generated.getCatalogCharts).toHaveBeenCalledWith({
      query: { cluster_id: "cluster-1", project_id: undefined, limit: 60, offset: undefined },
      signal: undefined,
    });
    ...
    expect(generated.getCatalogChartsByIdVersions).toHaveBeenCalledWith({
      path: { id: "chart-1" }, query: { cluster_id: "cluster-1", limit: 50 }, signal: undefined,
    });
    expect(generated.getCatalogChartsByIdValues).toHaveBeenCalledWith({
      path: { id: "chart-1" },
      query: { cluster_id: "cluster-1", version: "1.2.3" },
      signal: undefined,
    });
  });

  it("lets the server derive install ownership from cluster and namespace", async () => {
  ...
  it("does not require a project in the cluster install helper", async () => {
    vi.mocked(generated.postCatalogInstalled).mockResolvedValueOnce({
