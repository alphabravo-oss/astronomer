...
const EVENT_CASES: Record<string, QueryKey[]> = {
  ...
  "tool_operation.changed": [
    qk.tools.operation(EID),
    qk.tools.clusterStatus(CID),
  ],
  ...
  "admin_queue.changed": [
    qk.adminOperations.queues,
    qk.adminOperations.dlq(EID),
  ],
  "siem_forwarder.changed": [qk.siemForwarders.all],
  "cluster_agents.changed": [qk.agents.all, qk.agents.operations(CID)],
  "delivery_source.changed": [qk.delivery.sourcesAll(PID)],
  "component_bundle.changed": [qk.delivery.bundlesAll(PID)],
  "delivery_target.changed": [qk.delivery.targetsAll(PID)],
  "delivery_rollout.changed": [
...
describe("EVENT_ROUTES", () => {
  it("covers every published wire type (exhaustiveness)", () => {
    for (const type of WIRE_TYPES) {
      expect(
        EVENT_ROUTES[type],
        `missing EVENT_ROUTES entry for ${type}`,
      ).toBeTypeOf("function");
    }
  });
  ...
  it("has no route without an expected-keys row in this test", () => {
    ...
    expect(Object.keys(EVENT_ROUTES).sort()).toEqual(
      Object.keys(EVENT_CASES).sort(),
    );
  });

  it("every route emits only factory-built keys", () => {
    for (const [type, expected] of Object.entries(EVENT_CASES)) {
      const got = EVENT_ROUTES[type](
        fixture(type === "cluster.k8s_changed" ? "Pod" : undefined),
      );
  ...
  it("routes alerting.changed per payload kind (P4.9)", () => {
    ...
    expect(route({ ...fixture(), kind: "silence" })).toEqual([
      qk.alerting.silences,
    ]);
    expect(route({ ...fixture(), kind: "baseline" })).toEqual([
      qk.anomalyBaselines.all,
    ]);
  });
...
describe("K8S_KIND_ROUTES", () => {
  it("maps every informer kind to its factory keys", () => {
    expect(Object.keys(K8S_KIND_ROUTES).sort()).toEqual(
      Object.keys(KIND_CASES).sort(),
    );
    for (const [kind, expected] of Object.entries(KIND_CASES)) {
      expect(
        K8S_KIND_ROUTES[kind](CID, fixture(kind)),
        `keys for kind ${kind}`,
      ).toEqual(expected);
    }
  });
