...
async function mockApi(page: Page) {
  await page.route("**/api/v1/**", async (route) => {
    ...
    // this seed alone.
    if (
      path === `/clusters/${CLUSTER_ID}/k8s/api/v1/pods` &&
      method === "GET"
    ) {
    ...
    if (
      path ===
      `/clusters/${CLUSTER_ID}/k8s/api/v1/namespaces/${POD_NS}/pods/${POD_NAME}`
    ) {
...
test("pod drilldown: row opens pod detail with containers + Logs tab renders", async ({
  context,
  page,
}) => {
  ...
  await expect(page).toHaveURL(
    new RegExp(`/dashboard/clusters/${CLUSTER_ID}/pods/${POD_NS}/${POD_NAME}$`),
  );
  ...
  await expect(page.getByRole("tab", { name: "Exec" })).toBeVisible();
});
