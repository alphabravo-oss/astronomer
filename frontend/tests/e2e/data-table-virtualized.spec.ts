...
test("DataTable(virtualized): windows a 1200-row CIS findings list", async ({
  context,
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: `Findings (${FINDING_COUNT})` }),
  ).toBeVisible();
  ...
  await expect(
    grid.getByText(`1.2.${String(FINDING_COUNT).padStart(4, "0")}`),
  ).toBeVisible();
