// Crawl the Astronomer app with Playwright and capture full-page 2x
...
async function shoot(page, pathname, theme) {
  ...
  await page.screenshot({
    path: path.join(dir, slug(pathname) + ".png"),
    fullPage: true,
  });
...
async function crawlPass(ctx, seeds, theme, { follow, order, seen }) {
  ...
  const pages = await Promise.all(
    Array.from({ length: CONC }, () => ctx.newPage()),
  );
  async function worker(page) {
    while (true) {
      ...
      try {
        ...
        order.push(p);
        if (follow)
          for (const l of await links(page)) if (!seen.has(l)) queue.push(l);
      } catch (e) {
...
async function run() {
  ...
  // DARK: concurrent BFS from the dashboard; discovers routes.
  console.log("=== dark (crawl) ===");
  const seen = new Set();
  const order = [];
  const dctx = await newCtx("dark", storageState);
  await crawlPass(dctx, ["/dashboard"], "dark", { follow: true, order, seen });
  await dctx.close();
  ...
  await crawlPass(lctx, order, "light", {
    follow: false,
    order: [],
    seen: new Set(),
  });
  ...
  for (const theme of ["dark", "light"]) {
    ...
    await crawlPass(actx, AUTH, theme, {
      follow: false,
      order: [],
      seen: new Set(),
    });
